...
TEST(UrlResolverResolve, SearchQueryEncodesCorrectly) {
    const std::string url = UrlResolver::resolve(
        "what is C++", "https://search.brave.com/search?q=");
...
TEST(UrlResolverEncode, AlphanumericUnchanged) {
    const std::string encoded = UrlResolver::encode_search_query("OpenBrowser123");
