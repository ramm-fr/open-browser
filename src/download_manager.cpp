...
void DownloadManager::start_download(WebKitDownload* wd,
    ...
    if (!destination.empty()) {
        ...
        dl.destination = SettingsManager::instance().download_path()
                         + "/" + dl.filename;
    }
...
void DownloadManager::pause_download(int64_t id) {
    ...
    if (!dl || !dl->webkit_download) return;
...
void DownloadManager::set_progress_callback(std::function<void(const Download&)> cb) {
    progress_cb_ = std::move(cb);
}

void DownloadManager::set_completion_callback(std::function<void(const Download&)> cb) {
...
void DownloadManager::on_received_data(WebKitDownload* wd,
                                        guint64         /*length*/,
                                        gpointer        user_data) {
    ...
    dl->received_bytes = static_cast<int64_t>(
    ...
    if (response) {
        ...
        if (len > 0) dl->total_bytes = len;
        const char* mime = webkit_uri_response_get_mime_type(response);
        if (mime) dl->mime_type = mime;
    }

    if (self->progress_cb_) self->progress_cb_(*dl);
}
...
void DownloadManager::on_finished(WebKitDownload* wd, gpointer user_data) {
    ...
    if (self->completion_cb_) self->completion_cb_(*dl);
}

void DownloadManager::on_failed(WebKitDownload* wd,
                                 GError*        error,
                                 gpointer       user_data) {
    auto* self = static_cast<DownloadManager*>(user_data);
    Download* dl = self->find_by_webkit(wd);
    ...
    if (error && error->domain == WEBKIT_DOWNLOAD_ERROR &&
        ...
        if (error) {
            std::cerr << "[DownloadManager] Download failed: "
                      << error->message << "\n";
        }
    }
