...
constexpr std::array<std::string_view, 10> kKnownSchemes = {
    ...
    "data:",   "blob:",    "openbrowser://", "about:",  "javascript:"};
...
bool looks_like_domain(const std::string &s) {
  ...
  while (std::getline(stream, part, '.')) {
    if (!is_valid_hostname_label(part))
      return false;
  }
  return true;
}
...
bool is_localhost(const std::string &s) {
  ...
  if (s.starts_with("localhost:"))
    return true;
  // IPv4
  if (s.starts_with("127.") || s.starts_with("0.0.0.0"))
    return true;
  // IPv6
  if (s.starts_with("[::1]") || s.starts_with("::1"))
    return true;
  return false;
}
...
bool UrlResolver::looks_like_url(const std::string &input) {
  ...
  const std::string host_part =
      (slash != std::string::npos) ? input.substr(0, slash) : input;
