  ...
  try {
    ...
    if (j.contains("bookmarks") && j["bookmarks"].is_array()) {
      for (const auto &item : j["bookmarks"]) {
        ...
        if (bm.id >= next_bookmark_id_)
          next_bookmark_id_ = bm.id + 1;
    ...
    if (j.contains("folders") && j["folders"].is_array()) {
      for (const auto &item : j["folders"]) {
        ...
        if (folder.id >= next_folder_id_)
          next_folder_id_ = folder.id + 1;
...
void BookmarkManager::save() const {
  ...
  try {
    ...
    for (const auto &bm : bookmarks_) {
      j["bookmarks"].push_back({{"id", bm.id},
                                {"url", bm.url},
                                {"title", bm.title},
                                {"folder", bm.folder},
    ...
    for (const auto &folder : folders_) {
      j["folders"].push_back({{"id", folder.id},
                              {"name", folder.name},
...
Bookmark BookmarkManager::add(const std::string &url, const std::string &title,
...
bool BookmarkManager::update(int64_t id, const std::string &title,
                             const std::string &folder) {
  auto it = std::find_if(bookmarks_.begin(), bookmarks_.end(),
                         [id](const Bookmark &b) { return b.id == id; });
  if (it == bookmarks_.end())
...
std::optional<Bookmark>
BookmarkManager::find_by_url(const std::string &url) const {
  auto it = std::find_if(bookmarks_.begin(), bookmarks_.end(),
