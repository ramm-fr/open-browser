  ...
  if (!filter_dir.empty() && std::filesystem::exists(filter_dir)) {
    for (const auto &entry : std::filesystem::directory_iterator(filter_dir)) {
      if (entry.path().extension() == ".txt") {
        ...
        const bool tracker_only = entry.path().filename().string().find(
                                      "privacy") != std::string::npos;
