...
static const SecretSchema kPasswordSchema = {
    ...
    0,    // reserved
    nullptr, nullptr, nullptr, nullptr, nullptr, nullptr, nullptr  // reserved1-7
};
...
std::optional<std::string> PasswordManager::get_password(
...
std::vector<PasswordEntry> PasswordManager::get_for_origin(
    const std::string& origin) const {
    std::vector<PasswordEntry> result;
    for (const auto& e : entries_) {
        if (e.origin == origin) result.push_back(e);
    }
    return result;
}
...
bool PasswordManager::update_password(int64_t id, const std::string& new_password) {
    auto it = std::find_if(entries_.begin(), entries_.end(),
        [id](const PasswordEntry& e) { return e.id == id; });
    if (it == entries_.end()) return false;
...
std::vector<PasswordEntry> PasswordManager::find_matching(
...
bool PasswordManager::store_in_keyring(const std::string& label,
    ...
    gboolean result = secret_password_store_sync(
        ...
        &error,
        "origin",   label.c_str(),
        "username", label.c_str(),
        nullptr
    );

    if (error) {
        std::cerr << "[PasswordManager] Keyring store error: "
                  << error->message << "\n";
        g_error_free(error);
        return false;
    }
...
std::optional<std::string> PasswordManager::retrieve_from_keyring(
...
std::string PasswordManager::make_secret_label(const std::string& origin,
